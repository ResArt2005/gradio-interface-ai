...
def on_login_click(username, password, request: gr.Request):
    ...
    try:
        ...
        return "Вход успешен", gr.update(True), user_id, gr.update(visible=False), gr.update(visible=True), avatar_url, first_name, last_name, surname, e_mail
...
def on_logout_click(auth_state):
    ...
    return '', '', gr.update(False), gr.update(None), gr.update(visible=True), gr.update(visible=False), gr.update(None), gr.update(''), gr.update(''), gr.update(''), gr.update(''), gr.update('')